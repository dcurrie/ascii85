...
#if __ICCARM__
#define LCUT_MAX_NAME_LEN 32
#define LCUT_MAX_STR_LEN 32
#else
...
#define LCUT_LOGO \
"*********************************************************\n\
\tLCUT -- Lightweight C Unit Tests\n\
*********************************************************"

#define SUCCESS_TIP_FMT "\t\tCase '%s': \033[32mPassed\033[0m\n"
#define FAILURE_TIP_FMT "\t\tCase '%s': \033[31mFailure in %s, line %d of file %s, %s\033[0m\n"
...
typedef struct lcut_t {
    lcut_test_t *cut_test;
    int          cut_status;
    int          cut_result;
} lcut_t;
 ...
 *
 * t -- lcut_t*
 * s -- logical test description
 * setup -- setup fixture function
 * teardown -- teardown fixture function
 */
#define LCUT_TEST_BEGIN(t, s, setup, teardown) \
    (t)->cut_test      = NULL; \
    (t)->cut_result    = TEST_CASE_SUCCESS; \
    if (((t)->cut_status = lcut_test_init(&(t)->cut_test, (s), (setup), (teardown))) != 0) { \
        printf("[LCUT]: test init failed!, errcode[%d]\n", (t)->cut_status); \
        exit(1); \
    }

#define LCUT_TEST_END(t) do { \
        lcut_test_destroy(&(t)->cut_test); \
    } while(0)
 ...
 * teardown -- teardown fixture function
 */
#define LCUT_TS_INIT(t, p, s, setup, teardown) do { \
        if (((t)->cut_status = lcut_ts_init(&(p), (s), (setup), (teardown))) != 0) { \
 ...
 */
#define LCUT_TS_ADD(t, p) do { \
        lcut_ts_add((t)->cut_test, (p)); \
    } while(0)
 ...
 * e -- extra parameter
 */
#define LCUT_TC_ADD(t, p, s, f, e, before, after) do { \
        if (((t)->cut_status = lcut_tc_add((p), (s), (f), (e), (before), (after))) != 0) { \
 ...
 */
#define LCUT_TEST_RUN(t) do { \
        lcut_test_run((t)->cut_test, &(t)->cut_result); \
    } while(0)

/*
 * Report the result of the logical test
 */
#define LCUT_TEST_REPORT(t) do { \
        lcut_test_report((t)->cut_test); \
    } while(0)
